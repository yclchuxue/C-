...
int main()
{
    ...
    saddr.sin_port = htons(9998);              //端口号9998
        ...
        for (i = 0; i < nready; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    auto status = cudaIpcOpenMemHandle(&P, data.ptr, cudaIpcMemLazyEnablePeerAccess);
                    ...
                    int b;
                    status = cudaMemcpy(&b, P, sizeof(int), cudaMemcpyDeviceToHost);
                    if (status != cudaSuccess)
                    {
                    ...
                    status = cudaIpcCloseMemHandle(P);
